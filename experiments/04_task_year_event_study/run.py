#!/usr/bin/env python3
"""Task-year event study for AI-exposed vs low-exposure ATUS tasks.

Model:

    minutes_task,t = task FE + year FE
                     + sum_y beta_y * AI_exposed_task * 1[year=y]
                     + error_task,t

The outcome is a TUFNWGTP-weighted annual mean of minutes/day for each task.
The omitted baseline year is 2022. This is a dynamic version of the simple
task-year DiD and is mainly useful for checking pre-trends.
"""
...
def write_report(event: pd.DataFrame, panel: pd.DataFrame) -> None:
    ...
    for row in event.itertuples(index=False):
        rows.append(
            f"| {row.year} | {row.coef:.3f} | {row.cluster_se_by_task:.3f} | {row.cluster_p_norm:.3f} |"
        )
    table = "\n".join([
    ...
    report = f"""# Task-Year Event Study
...
- AI-exposed: {int(panel.loc[panel['treated_ai'].eq(1), 'activity_code'].nunique())}
- Low-exposure physical: {int(panel.loc[panel['treated_ai'].eq(0), 'activity_code'].nunique())}

Task-year observations in model years: {int(panel[panel['year'].isin(MODEL_YEARS)].shape[0])}
